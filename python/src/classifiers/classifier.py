...
from ..types import ConversationMessage, AgentTypes, TemplateVariables
...
class Classifier(ABC):
    def __init__(self):
        self.default_agent = BedrockLLMAgent(
            ...
            streaming=True,
            description="A knowledgeable generalist capable of addressing a wide range of topics. This agent should be selected if no other specialized agent is a better fit."
        ))
...
"""
    ...
    def replace_placeholders(template: str, variables: TemplateVariables) -> str:
        import re
        return re.sub(r'{{(\w+)}}', lambda m: '\n'.join(variables.get(m.group(1), [m.group(0)])) if isinstance(variables.get(m.group(1)), list) else variables.get(m.group(1), m.group(0)), template)
