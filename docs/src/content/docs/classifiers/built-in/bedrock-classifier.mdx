...
- Supports AWS CLI profile configuration for improved local development experience
...
<Tabs syncKey="runtime">
  ...
  <TabItem label="Python" icon="seti:python">
    ...
    custom_bedrock_classifier = BedrockClassifier(BedrockClassifierOptions(
        ...
        aws_profile_name="dev"  # New parameter for specifying AWS CLI profile
    ))
...
- `aws_profile_name` (optional): The name of the AWS CLI profile to use for authentication. This is particularly useful for local development with MFA-enabled profiles.
...
4. **AWS Profile**: When developing locally, especially with MFA-enabled accounts, use the `aws_profile_name` parameter to specify the appropriate AWS CLI profile.
